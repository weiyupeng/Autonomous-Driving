...
class Tub(object):
    ...
    def get_train_gen(self, X_keys, Y_keys,
        ...
        def random_augment(image,angle):
            for each in batch:
                ...
                if np.random.rand() < 0.5:
                    image, angle = img_random_flip(image, angle)
                return image, angle       
            ...
            if data_type == 'train':
                for i in range(len(X)):
                    for j in range(len(X[i])):
                        X[i][j],Y[i][j] = random_augment(X[i][j],Y[i][j])
                        #print('---------',X[i][j])
                        #print('---------',Y[i][j])
            yield X, Y
    ...
    def get_train_val_gen(self, X_keys, Y_keys, batch_size=128, train_frac=.8,
        ...
        train_gen = self.get_train_gen(X_keys=X_keys, Y_keys=Y_keys, batch_size=batch_size,
                                       record_transform=train_record_transform, df=train_df,data_type='train')
